...
def auth_heat(username, password, endpoint, tenant_name, os_cacert):
        try:
            keystone_client = ksclient.Client(username=username, password=password, auth_url=endpoint, project_name=tenant_name, cacert=os_cacert)
        except ValueError:
...
#create Stack
def stack_create_heat(username, password, endpoint, tenant_name, heat_url, nct_template, stack_name, os_cacert):
    auth_heat(username, password, endpoint, tenant_name, os_cacert)
    get_heat(heat_url)
...
# Delete Stack
def stack_delete_heat(username, password, endpoint, tenant_name, heat_url, stack_uuid, os_cacert):
    auth_heat(username, password, endpoint, tenant_name, os_cacert)
    get_heat(heat_url)
...
# Status Stack
def stack_status_heat(username, password, endpoint, tenant_name, heat_url, stack_uuid, os_cacert):
    auth_heat(username, password, endpoint, tenant_name, os_cacert)
    get_heat(heat_url)
...
# Output Stack
def stack_output_heat(username, password, endpoint, tenant_name, heat_url, stack_uuid, output_template, os_cacert):
    auth_heat(username, password, endpoint, tenant_name, os_cacert)
    get_heat(heat_url)
